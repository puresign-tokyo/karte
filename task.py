...
def main_task(settings: dict):
    ...
    for page in range(settings["max_page"]):
        ...
        for parsed_res_get_replay in parsed_res_get_replays:
            ...
            ) - replay_uploaded_at > timedelta(days=settings["search_days_back"]):
                logger.info(
            ...
            if parsed_res_get_replay["game_id"] not in settings["games"].keys():
                logger.error(
                    f"Encounted unknown game: {parsed_res_get_replay["game_id"]}"
                )
                continue
            ...
            if not exists_key_in_responce(parsed_res_get_replay, "replay_id"):
                continue
            try:
                res_replay_file = requests.get(
                    url=f"{settings["backend_url"]}/replays/{parsed_res_get_replay["replay_id"]}/file"
                )
                res_replay_file.raise_for_status()
                logger.info(
                    f"requested replay binary file to L-uploader, and it responced. url: {url}"
                )
            ...
            game_path = Path(
                settings["games"][parsed_res_get_replay["game_id"]]["path"]
            )
            # 0始まりにして0の場合はスキップさせる
            # リネーム制限しない場合の処理をリネーム処理と同等に扱うため
            for storage_file_id in range(
                settings["games"][parsed_res_get_replay["game_id"]][
                ...
                cur_replay_file = (
                    game_path
                    / f"{parsed_res_get_replay["game_id"]}_{storage_file_id:02}.rpy"
                )
